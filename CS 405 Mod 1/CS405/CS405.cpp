...
bool status = false; // Initial overflow status is false
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        //more than the max value will cause overflow
        if ((increment >= 0) && (result > (max - increment))) {
            // Overflow status is true
            status = true;
        }
        //less than the min value will cause underflow
        if ((increment < 0) && (result < (min - increment))) {
            // Overflow status is true
            status = true;
        }
        result += increment;
...
void test_overflow()
{
    ...
    // Adding WITHOUT Overflow Test
    std::cout << "\tAdding Numbers Without Overflow (" << +start << ", " << +increment << ", " << steps << ") = ";
    auto result = add_numbers<T>(start, increment, steps);

    if (status == false) { // Overflow did not occur
        std::cout << "No Overflow - Result: " << +result << std::endl;
    }
    else {
        std::cout << "Overflow Occured" << std::endl; // Overflow occurred
        status = false; // Reset overflow status
    }

    // Adding WITH Overflow Test
...
void test_underflow()
{
    ...
    // Subtraction WITHOUT Overflow Test
    std::cout << "\tSubtracting Numbers Without Overflow (" << +start << ", " << +decrement << ", " << steps << ") = ";
    auto result = subtract_numbers<T>(start, decrement, steps);

    if (status == false) { // Underflow did not occur
        std::cout << "No Underflow - Result: " << +result << std::endl;
    }
    else { // Underflow occured cause less
        std::cout << "Underflow Occurred" << std::endl;
        status = false; // Reset overflow status
    }

    // Subtraction WITH Underflow Test
